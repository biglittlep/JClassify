...
% Last Modified by GUIDE v2.5 18-Jul-2016 13:21:33
...
if isfield(handles,'myclasses') && isfield(handles,'mydata')
    ...
    if numclasses > 1
        ...
        % save output
        handles.outputdata = gdata;
        handles.outputclasses = gclasses;
        ...
        % report results
        res = {['Sensitivity: ' num2str(output.sen) '%'], ...
            ['Specificity: ' num2str(output.spe) '%'], ...
            ['Accuracy: ' num2str(output.acc) '%'], ...
            ['Mean Accuracy: ' num2str(output.accmean) '%'], ...
            '', ...
        ...
        axes(handles.graph2);
        cla
        if length(unique(gclasses)) == 2
            viewroc(gclasses,handles.output.scores(:,2));
            handles.ROCclasses = gclasses;
            handles.ROCscores = handles.output.scores(:,2);
        end
...
% ROC
if isfield(handles,'ROCclasses')
    myfig(6,6);
    viewroc(handles.ROCclasses,handles.ROCscores);
end
...
help_str = {'This toolbox is designed for classification using supervised machine learning. To begin:', ...
    '1. Load a data matrix (#samples by #features) and classes vector (#samples by 1) into the Matlab workspace.', ...
    '2. Run jclassify in Matlab, which will open the GUI. Select the data and classes variables from the lists (top left).', ...
    '3. Below that, select at least 2 classes you want to use for classification (hold CTRL to select more than one).', ...
    '4. Change any parameters in the ''Classification'' box, or leave them on Auto, then press Classify.', ...
    '5. Once the classification has finished, the results will display to the right (performance metrics, ROC, parameter optimization).'};
h = helpdlg(help_str,'Help');
uiwait(h);

help_str = {'When in doubt, leave inputs on Auto and default values will be used. Custom parameter information follows:', ...
    '1. Classifier - which algorithm/model to use for classification, you can also add your own in the classification/classifiers folder', ...
    '2. Feature Selection - how to reduce the number of features used for classification, you can also add your own in the classification/feature selection folder', ...
    '3. Parameters - custom parameters for the classifier, the first parameter is always the number of features, other parameters will depend on the classifier', ...
    'Examples...', ...
    'For support vector machines you could use {200}', ...
    'This would test using 200 features.', ...
    'For boosted trees you could use {[50:50:600], [2 4 8 16]}', ...
    'This would test using number of features of 50, 100, ... 600 and using number of trees of 2, 4, 8, 16.', ...
    '4. Cross Validation Folds - the number of partitions to randomly split the data into training/testing sets', ...
    '5. Partition - this allows you to specify a custom partition (for instance per patient), using a vector the same size as classes'};
helpdlg(help_str,'Help');
